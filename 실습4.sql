...
BEGIN
    VI_NUM := 100;          -- 실행부
    VI_NUM := 1000;
    -- CO_NUM := 2000;      -- 상수의 값은 최초에 한번만 초기화하고 바꿀 수 없다.
...
-- 조회하여 1,000,000 원 이상 2,000,000 원 이하이면 1을 출력
--        2,000,001 원 이상 3,000,000 원 이하이면 2를 출력
--        그렇지 않으면 3을 출력해보시오
DECLARE
    -- 지정된 부서 D1
...
BEGIN
    ...
    INTO MAX_SALARY
    FROM EMPLOYEE
    WHERE E.DEPT_CODE = VN_DEPT_CODE;
...
(
    ...
    IN_CONTENT IN VARCHAR2 DEFAULT '제목없음'
)
IS
...
EXECUTE PRO_EMP_WRITER('220', '글 제목3', NULL);
...
-- 프로시저 호출
-- 1) 매개 변수 없거나, IN 매개변수만 : EXCUTE 프로시저명(인자1, 인자2);
...
BEGIN
    ...
    -- 예외처리부
    EXCEPTION
        -- NO_DATA_FOUND : SELECT INTO 변수를 사용할 때, 
        -- 조회 결과가 하나도 없는 경우 예외 발생
